  ...
  await prismaClient.$transaction(async (tx: any) => {
    ...
    const combosCategory = await tx.menuCategory.create({
      data: {
        name: "Combos",
        slug: "combos",
        description: "Combos especiais com hambúrguer, acompanhamento e bebida",
    ...
    const hamburguersCategory = await tx.menuCategory.create({
      data: {
        name: "Lanches",
        slug: "lanches",
        description: "Hambúrgueres e lanches deliciosos",
        restaurantId: restaurant.id,
      },
    });
    await tx.product.createMany({
    ...
    const frenchFriesCategory = await tx.menuCategory.create({
      data: {
        name: "Fritas",
        slug: "fritas",
        description: "Batatas fritas crocantes em diferentes tamanhos",
        restaurantId: restaurant.id,
      },
    });
    await tx.product.createMany({
    ...
    const drinksCategory = await tx.menuCategory.create({
      data: {
        name: "Bebidas",
        slug: "bebidas",
        description: "Bebidas geladas para acompanhar seu lanche",
        restaurantId: restaurant.id,
      },
    });
    await tx.product.createMany({
    ...
    const desertsCategory = await tx.menuCategory.create({
      data: {
        name: "Sobremesas",
        slug: "sobremesas",
        description: "Sobremesas doces para finalizar sua refeição",
        restaurantId: restaurant.id,
      },
    });
    await tx.product.createMany({
